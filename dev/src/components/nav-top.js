...
export class NavTop extends LitElement {
    ...
    return html`
      <nav>
        ...
        <div class="nav-buttons">
          <a href="/list">${this.renderUserSvg()}${msg(html`Employee List`)}</a>
          <a href="/add">${this.renderPlusSvg()}${msg(html`Add Employee`)}</a>
          <button @click="${this._handleChangeLanguage}">
            ${getLocale() === 'en' ? this.renderUSAFlagSvg() : this.renderTRFlagSvg()}
          </button>
        </div>
      </nav>
    `;
  ...
  async _handleChangeLanguage() {
    // employeeService.send({type: 'CHANGE_LANGUAGE', language: 'tr'});
    const currentLocale = getLocale();
    this.dispatchEvent(
      new CustomEvent('locale-changed', {bubbles: true, detail: currentLocale})
    );
  }
